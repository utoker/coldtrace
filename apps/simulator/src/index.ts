...
class VaccineSimulator {
  ...
  constructor() {
    ...
    try {
      ...
      console.log(chalk.green('✅ Interactive controls initialized - keyboard commands ready'));
    } catch (error) {
      console.error(chalk.red('❌ Failed to setup interactive controls:'), error);
  ...
  private setupInteractiveControls(): void {
    ...
    this.rl.on('line', (input: string) => {
      ...
      console.log(chalk.magenta(`[DEBUG] Received command: "${command}" (length: ${command.length})`));
    ...
    console.log(chalk.green('✅ Interactive controls ready - try typing "h" for help'));
  }

  private async initialize(): Promise<void> {
    console.log(chalk.blue.bold('\n💉 ColdTrace Vaccine Monitoring Simulator'));
    console.log(chalk.blue('========================================'));
    console.log(chalk.gray(`GraphQL Endpoint: ${process.env.GRAPHQL_ENDPOINT || 'http://localhost:4000/graphql'}`));
    ...
    try {
      ...
      console.log(chalk.green(`✅ Connected to GraphQL at ${process.env.GRAPHQL_ENDPOINT || 'http://localhost:4000/graphql'}`));
  ...
  private async loadDevicesFromBackend(): Promise<void> {
    ...
    try {
      ...
      if (result.data?.getDevices) {
        ...
        console.log(chalk.green(`✅ Fetched ${devices.length} devices from database`));
  ...
  private generateVaccineTemperature(device: DeviceState): number {
    ...
    if (!device.isOnline) {
      ...
      return Math.round((device.targetTemperature + (Math.random() - 0.5) * 0.5) * 100) / 100;
    }
  ...
  private calculateReadingStatus(temperature: number, minTemp: number, maxTemp: number): string {
  ...
  private async sendReadingWithRetry(deviceId: string, input: { deviceId: string; temperature: number; battery: number }, maxRetries = 3): Promise<Reading | null> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt === maxRetries) {
          console.error(
            chalk.red(`❌ Failed to send reading for ${deviceId} after ${maxRetries} attempts:`),
  ...
  private startSimulation(): void {
    ...
    console.log(chalk.blue('📊 Sending readings every 5 seconds...\n'));
    
    this.intervalId = setInterval(async () => {
      if (this.isShuttingDown) return;
      
      const promises = Array.from(this.devices.entries()).map(async ([_deviceId, device]) => {
        // Update battery level (decreases by 0.01% per reading) - only for online devices
        if (device.isOnline) {
        ...
        const status = this.calculateReadingStatus(temperature, device.minTemp, device.maxTemp);
        ...
        const reading = await this.sendReadingWithRetry(device.id, readingInput);
      ...
      // Show periodic statistics every 10 readings
      if (this.stats.totalReadings > 0 && this.stats.totalReadings % (this.devices.size * 2) === 0) {
        const successRate = Math.round((this.stats.successfulReadings / this.stats.totalReadings) * 100);
        console.log(chalk.blue(`📈 Total readings sent: ${this.stats.totalReadings}, Success rate: ${successRate}%`));
      }
  ...
  private displayFinalStats(): void {
    ...
    const successRate = this.stats.totalReadings > 0 
      ? Math.round((this.stats.successfulReadings / this.stats.totalReadings) * 100) 
      : 0;

    console.log(chalk.blue('\n📊 Simulation Statistics:'));
    console.log(chalk.gray('========================'));
    console.log(chalk.green(`✅ Successful readings: ${this.stats.successfulReadings}`));
    console.log(chalk.red(`❌ Failed readings: ${this.stats.failedReadings}`));
    console.log(chalk.yellow(`⚠️  Alerts created: ${this.stats.alertsCreated}`));
  ...
  private showControls(): void {
    ...
    console.log(chalk.gray('═══════════════════════════'));
    console.log(chalk.yellow('  e + Enter') + chalk.gray(' - Trigger temperature excursion on random device'));
    console.log(chalk.yellow('  b + Enter') + chalk.gray(' - Simulate low battery on random device'));
    console.log(chalk.yellow('  o + Enter') + chalk.gray(' - Take random device offline'));
    console.log(chalk.yellow('  p + Enter') + chalk.gray(' - Simulate power outage (all devices offline)'));
    console.log(chalk.yellow('  a + Enter') + chalk.gray(' - Simulate batch arrival (bring 3 devices online)'));
    console.log(chalk.yellow('  r + Enter') + chalk.gray(' - Reset all devices to normal'));
    console.log(chalk.yellow('  s + Enter') + chalk.gray(' - Show current statistics'));
  ...
  private getRandomOnlineDevice(): DeviceState | null {
    ...
    return onlineDevices[Math.floor(Math.random() * onlineDevices.length)] || null;
  }
  ...
  private triggerExcursion(): void {
    ...
    console.log(chalk.red(`🌡️  EXCURSION: ${device.name} temperature spiking to 12°C!`));
    ...
    setTimeout(() => {
      if (device.demoMode === 'excursion') {
        ...
        console.log(chalk.green(`✅ ${device.name} excursion resolved, returning to normal`));
      }
    }, 5 * 60 * 1000);
  }
  ...
  private simulateLowBattery(): void {
    const device = this.getRandomOnlineDevice();
    if (!device) {
      console.log(chalk.red('❌ No online devices available for battery simulation'));
      return;
    }
    ...
    console.log(chalk.yellow(`🔋 LOW BATTERY: ${device.name} battery at ${device.battery.toFixed(1)}%`));
  }
  ...
  private simulatePowerOutage(): void {
    ...
    if (onlineDevices.length === 0) {
      console.log(chalk.red('❌ No online devices for power outage simulation'));
      return;
    }
    ...
    console.log(chalk.red(`⚡ POWER OUTAGE: All ${onlineDevices.length} devices went offline!`));
    ...
    setTimeout(() => {
      ...
      console.log(chalk.green(`🔌 POWER RESTORED: ${onlineDevices.length} devices back online`));
    }, 30 * 1000);
  }

  private simulateBatchArrival(): void {
    const offlineDevices = Array.from(this.devices.values()).filter(d => !d.isOnline);
    ...
    console.log(chalk.green(`📦 BATCH ARRIVAL: ${devicesToActivate.length} devices came online simultaneously`));
    devicesToActivate.forEach(device => {
      console.log(chalk.gray(`  📱 ${device.name} - ${device.battery.toFixed(1)}% battery`));
    });
  }
  ...
  private returnToNormal(): void {
    ...
    Array.from(this.devices.values()).forEach(device => {
      if (device.demoMode !== null || device.isInExcursion || !device.isOnline) {
        device.isOnline = true;
        device.isInExcursion = false;
        device.targetTemperature = 5.0;
        device.demoMode = null;
    ...
    if (changesCount > 0) {
      console.log(chalk.green(`🔄 RESET: ${changesCount} devices returned to normal operation`));
    } else {
  ...
  private showCurrentStats(): void {
    ...
    const onlineDevices = Array.from(this.devices.values()).filter(d => d.isOnline);
    const offlineDevices = Array.from(this.devices.values()).filter(d => !d.isOnline);
    const excursionDevices = Array.from(this.devices.values()).filter(d => d.isInExcursion);
    const lowBatteryDevices = Array.from(this.devices.values()).filter(d => d.battery < 20);
    
    console.log(chalk.gray('\n🏥 Device Status Overview:'));
    console.log(chalk.green(`  📱 Online devices: ${onlineDevices.length}`));
    console.log(chalk.red(`  📴 Offline devices: ${offlineDevices.length}`));
    console.log(chalk.yellow(`  🌡️  Temperature excursions: ${excursionDevices.length}`));
    console.log(chalk.hex('#FFA500')(`  🔋 Low battery devices: ${lowBatteryDevices.length}`));
    console.log();
  }
