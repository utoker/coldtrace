    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-1">
          <svg
            className={`w-5 h-5 ${getBatteryColor(device.battery)}`}
            fill="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      {temperature !== undefined ? (
        <div
          className={`rounded-lg p-4 ${tempColors?.bg} ${tempColors?.border} border`}
        >
          ...
          {device.latestReading?.timestamp && (
            <div className="mt-2 text-xs text-gray-500">
              Updated:{' '}
              {new Date(device.latestReading.timestamp).toLocaleTimeString()}
            </div>
          )}
        </div>
      ) : (
