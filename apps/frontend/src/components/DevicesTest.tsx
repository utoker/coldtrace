...
export function DevicesTest() {
  ...
  useEffect(() => {
    if (
      ...
      'temperatureUpdates' in tempData
    ) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      setRecentUpdates((prev) => [
        (tempData as any).temperatureUpdates,
        ...prev.slice(0, 4),
      ]);
    }
  }, [tempData]);
    ...
    <div className="p-6 max-w-6xl mx-auto">
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    {device.latestReading && (
                      <div className="flex justify-between text-sm">
                        ...
                        <span
                          className={`font-medium ${
                            device.latestReading.status === 'NORMAL'
                              ? 'text-green-600'
                              : 'text-orange-600'
                          }`}
                        >
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold mb-4">
          Real-time Updates (Subscription Test)
        </h2>
        {recentUpdates.length === 0 ? (
          <p className="text-gray-600">
            Waiting for real-time temperature updates...
          </p>
        ) : (
          <div className="space-y-3">
            {recentUpdates.map((update, index) => (
              <div
                key={`${update.id}-${index}`}
                className="border-l-4 border-blue-400 pl-4 py-2 bg-blue-50"
              >
